...
namespace PeerCastStation.FLV
{
  ...
  internal static class ExTagHeaderReader
  {
    ...
    public static bool TryReadFourCc(byte[] body, ref int pos, out FourCcCodec? codec, out string? name)
    {
      ...
      codec = FourCcRegistry.Lookup(value);
      name = codec?.Name ?? Encoding.ASCII.GetString(body, pos, 4);
      pos += 4;
      return true;
    }
    ...
    /// <summary>
    /// Multitrack サブヘッダ(multitrackType/実packetType、ManyTracksManyCodecs 以外は
    /// 共通 FourCC)を読む。形式は映像/音声で完全に同一なのでここに一本化する。
    /// バイトが不足する場合は false を返す。
    /// </summary>
    public static bool TryReadMultitrack(byte[] body, ref int pos, out int packet_type, out FourCcCodec? codec, out string? fourcc)
    {
      packet_type = 0;
      codec = null;
      fourcc = null;
      ...
      if (multitrack_type!=(int)AvMultitrackType.ManyTracksManyCodecs) {
        if (!TryReadFourCc(body, ref pos, out codec, out fourcc)) return false;
      }
      return true;
    }
  }
  ...
  internal readonly struct ExVideoTagHeader
  {
    ...
    /// <summary>既知コーデックの記述子。未知の FourCC は null(名前は <see cref="FourCc"/> に残る)。</summary>
    ...
    private ExVideoTagHeader(bool is_ex, int frame_type, VideoPacketType packet_type, FourCcCodec? codec, string? fourcc, bool is_multitrack, int payload_offset, int composition_time)
    {
      IsExHeader      = is_ex;
    ...
    public static bool TryParse(byte[] body, out ExVideoTagHeader result)
    {
      ...
      if ((b0 & 0x80)==0) {
        // レガシー(非 enhanced)タグ
        result = new ExVideoTagHeader(false, (b0>>4) & 0x07, default, null, null, false, -1, 0);
        return true;
      }
      ...
      FourCcCodec? codec = null;
      string? fourcc = null;
      int payload_offset = -1;
      int composition_time = 0;
      if (packet_type==(int)VideoPacketType.Multitrack) {
        is_multitrack = true;
        if (!ExTagHeaderReader.TryReadMultitrack(body, ref pos, out packet_type, out codec, out fourcc)) return false;
        ...
        if (!ExTagHeaderReader.TryReadFourCc(body, ref pos, out codec, out fourcc)) return false;
        // E-RTMP v2 仕様: CodedFrames で 24bit compositionTimeOffset を持つかは
        // コーデックの属性(FourCcRegistry の HasCompositionTime)で引く。
        // 持つコーデック(AVC/HEVC/VVC)で読み飛ばさないとフレーム先頭3バイトが欠落し、
        // 持たないコーデック(AV1/VP9 は body 先頭が即コーデックデータ)で読むと
        // フレーム先頭3バイトを CTS と誤読する。未知のコーデックは持たない扱い。
        // CodedFramesX は CTS=0(常に持たない)。
        if (packet_type==(int)VideoPacketType.CodedFrames && codec?.HasCompositionTime==true) {
          if (pos+3>body.Length) return false;
          composition_time = ExTagHeaderReader.ReadInt24Signed(body, pos);
          pos += 3;
        }
        payload_offset = pos;
      }

      result = new ExVideoTagHeader(true, frame_type, (VideoPacketType)packet_type, codec, fourcc, is_multitrack, payload_offset, composition_time);
      return true;
    }
  }
  ...
  internal readonly struct ExAudioTagHeader
  {
    ...
    private ExAudioTagHeader(bool is_ex, AudioPacketType packet_type, FourCcCodec? codec, string? fourcc, bool is_multitrack, int payload_offset)
    {
      IsExHeader    = is_ex;
      PacketType    = packet_type;
      Codec         = codec;
    ...
    public static bool TryParse(byte[] body, out ExAudioTagHeader result)
    {
      ...
      if (sound_format!=9) {
        // レガシー(非 enhanced)音声
        result = new ExAudioTagHeader(false, default, null, null, false, -1);
        return true;
      }
      var packet_type = b0 & 0x0F;
      var pos = 1;
      ...
      result = new ExAudioTagHeader(true, (AudioPacketType)packet_type, codec, fourcc, is_multitrack, payload_offset);
      return true;
    }
  }
