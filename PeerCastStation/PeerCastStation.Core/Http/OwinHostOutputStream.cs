    ...
    : OutputStreamBase
  {
    ...
    protected override async Task<StopReason> DoProcess(CancellationToken cancel_token)
    {
      ...
      while (!cancel_token.IsCancellationRequested && keep_count-->0) {
        ...
        try {
          await owinHost.Invoke(req, Connection, RemoteEndPoint as IPEndPoint, RemoteEndPoint as IPEndPoint, AccessControlInfo, env.Environment, cancel_token).ConfigureAwait(false);
        }
        catch (Exception ex) {
          Logger.Error(ex);
